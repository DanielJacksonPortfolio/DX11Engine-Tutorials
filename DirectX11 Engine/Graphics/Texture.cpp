...
Texture::Texture(ID3D11Device* device, const std::string& filePath, aiTextureType type)
{
	this->type = type;
	HRESULT hr;
	if (StringToolkit::GetFileExtension(filePath) == ".dds")
		hr = DirectX::CreateDDSTextureFromFile(device, StringToolkit::StringToWide(filePath).c_str(), this->texture.GetAddressOf(), this->textureView.GetAddressOf());
	else
		hr = DirectX::CreateWICTextureFromFile(device, StringToolkit::StringToWide(filePath).c_str(), this->texture.GetAddressOf(), this->textureView.GetAddressOf());
...
{
	this->type = type;
	CD3D11_TEXTURE2D_DESC textureDesc(DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM, width, height);
	...
	COM_ERROR_IF_FAILED(hr, "Failed to create shader resource view from texture generated");
}
